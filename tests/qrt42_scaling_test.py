...
Conditions (6, packed one per qubit, full 6-fold rotation):
  A1  10MHz @ 0.1x R0    amplitude-scaling arm (low)
  A2  10MHz @ 1x   R0    PRIMARY replication arm
  A3  10MHz @ 10x  R0    amplitude-scaling arm (high)
  PR  10MHz @ 1x, per-pulse random phase (seeded) — spectral-specificity control
  NB  11MHz @ 1x         neighbor control (primary comparator)
  REF undriven           descriptive reference

Primary index: G_rep = same-qubit paired difference contrast, envelope area,
A2 (10MHz@1x) vs NB (11MHz@1x). Decision (preregistered):
...
N_BOOT = 2000
SEED = 20260708
THRESHOLD_SCALE = 0.03   # amplified-arm threshold: drive-coupled scaling predicts
                         # >> +3% at 10x R0; artifact non-scaling predicts ~0
...
def analyze(rows, n_boot=N_BOOT, seed=SEED):
    ...
    g_scale = contrast(area, "A3_10MHz_10x", NBS)   # PRIMARY: amplified arm
    g_rep   = contrast(area, "A2_10MHz_1x", NBS)     # secondary: 1x replication/peak
    ...
    for _ in range(n_boot):
        ...
        boots.append(contrast(ba, "A3_10MHz_10x", NBS))
    lo, hi = np.percentile(boots, [2.5, 97.5])
    if g_scale >= THRESHOLD_SCALE and lo > 0: decision = "SCALES_WITH_AMPLITUDE"
    elif hi < THRESHOLD_SCALE: decision = "DOES_NOT_SCALE"
    else: decision = "INCONCLUSIVE"
    return {"G_scale_10x": g_scale, "ci95": [float(lo), float(hi)],
            "G_rep_1x": g_rep, "G_phase_rand": g_pr,
...
def run_ibm(device):
    ...
    json.dump(calib, open("calibration_snapshot_v42.json", "w"), indent=2, default=str)
    print("[calibration] saved -> calibration_snapshot_v42.json")
...
def main():
    ...
    a = analyze(rows)
    print(f"\nG_scale (10MHz@10x vs neighbors) = {a['G_scale_10x']*100:+.2f}%  "
          f"95% CI [{a['ci95'][0]*100:+.2f}%, {a['ci95'][1]*100:+.2f}%]")
    print(f"secondary: G_rep(1x)={a['G_rep_1x']*100:+.2f}%  "
          f"G(phase-rand)={a['G_phase_rand']*100:+.2f}%")
    print(f"=== DECISION (preregistered, v4.2) === {a['decision']}")
    out["analysis"] = a
    fn = "qrt42_hardware_results.json" if args.backend == "ibm" else "qrt42_sim_results.json"
